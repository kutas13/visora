...
export default function AdminVizeSonucTakipPage() {
  ...
  const load = useCallback(async () => {
    ...
    // Sonuç beklenen dosyalar: başvuru yapılmış (vize merkezine gitmiş) ve
    // henüz "islemden_cikti" olmamış olanlar. Müşterinin pasaportu vize
    // merkezinden çıkınca (islemden_cikti=true) bu listeden düşer.
    const { data, error } = await supabase
      .from("visa_files")
      .select(
        "id, musteri_ad, pasaport_no, hedef_ulke, takip_no, dogum_tarihi, randevu_tarihi, basvuru_yapildi, basvuru_yapildi_at, islemden_cikti, created_at, profiles:assigned_user_id(name)"
      )
    ...
    if (!error && data) {
      // profiles tek obje olarak donsun
      const fixed: Row[] = (data as unknown as Array<Record<string, unknown>>).map((d) => {
        const p = (d as { profiles?: unknown }).profiles;
        const profile = Array.isArray(p) ? (p[0] as { name: string } | null) ?? null : ((p as { name: string } | null) ?? null);
        return { ...(d as unknown as Row), profiles: profile };
      });
      setRows(fixed);
    }
    setLoading(false);
  ...
  const filtered = useMemo(() => {
    ...
    if (filterCountry !== "all") {
      list = list.filter((r) => r.hedef_ulke === filterCountry);
    }
    if (search.trim()) {
      ...
      list = list.filter(
        (r) =>
          ...
          norm(r.profiles?.name || "").includes(q)
      );
    }
    return list;
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-6">
        <div className="bg-white border border-slate-200 rounded-xl p-4 mb-4 flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[240px] max-w-[400px]">
            ...
            <input
              type="text"
              placeholder="Müşteri, pasaport, takip no veya personel ara…"
              value={search}
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          {loading ? (
            ...
            <div className="py-20 text-center">
              ...
              <p className="text-sm text-slate-500">Şu an takip edilen sonuç bekleyen dosya yok.</p>
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-100">
                  {filtered.map((r) => (
                    <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-slate-600">{r.profiles?.name || "-"}</td>
