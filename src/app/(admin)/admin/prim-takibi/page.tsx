...
import type { VisaFile, CommissionRate, Profile } from "@/lib/supabase/types";
...
function getPrimDate(f: VisaFile): string | null {
  return (f.prim_tarihi as string | null) || f.sonuc_tarihi;
}
...
export default function AdminPrimTakibiPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [filesRes, ratesRes] = await Promise.all([
      ...
      supabase.from("commission_rates").select("*").order("country", { ascending: true }),
    ]);

    setFiles((filesRes.data as VisaFileWithProfile[] | null) || []);
    setRates((ratesRes.data as CommissionRate[] | null) || []);
  ...
  const staffList = useMemo(() => {
    ...
    for (const f of files) {
      if (f.profiles?.id && f.profiles?.name) set.set(f.profiles.id, f.profiles.name);
    }
  ...
  const monthFiles = useMemo(() => {
    let scoped = selectedStaff === "all" ? files : files.filter((f) => f.assigned_user_id === selectedStaff);
    ...
    return scoped;
  }, [files, selectedStaff, selectedMonth]);
  ...
  // Personel bazında toplam
  const perStaffTotals = useMemo(() => {
    const map = new Map<string, { id: string; name: string; approved: number; eur: number; usd: number; tl: number }>();
    for (const e of monthEntries) {
      const key = e.staffId;
      let entry = map.get(key);
      if (!entry) {
        entry = { id: key, name: e.staffName, approved: 0, eur: 0, usd: 0, tl: 0 };
        map.set(key, entry);
      }
      entry.approved += 1;
      if (e.currency === "EUR") entry.eur += e.amount;
      else if (e.currency === "USD") entry.usd += e.amount;
      else entry.tl += e.amount;
    }
    return Array.from(map.values())
      .map((s) => {
        const eurTl = fx ? s.eur * fx.EUR : 0;
        const usdTl = fx ? s.usd * fx.USD : 0;
        return { ...s, eurTl, usdTl, grandTl: eurTl + usdTl + s.tl };
      })
      .sort((a, b) => b.grandTl - a.grandTl);
  }, [monthEntries, fx]);
  ...
  const saveEditDate = async () => {
    ...
    const { error } = await supabase
      .from("visa_files")
      .update({ prim_tarihi: editingDate || null } as Partial<VisaFile>)
  ...
  const resetPrimDate = async (f: VisaFile) => {
    ...
    const { error } = await supabase
      .from("visa_files")
      .update({ prim_tarihi: null } as Partial<VisaFile>)
      .eq("id", f.id);
    if (error) {
      ...
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-3 text-center px-6">
        <div className="w-14 h-14 rounded-xl bg-rose-50 border border-rose-200 text-rose-600 flex items-center justify-center">
          <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01M5 19h14a2 2 0 001.85-2.75L13.85 4.15a2 2 0 00-3.7 0L3.15 16.25A2 2 0 005 19z" />
          </svg>
        </div>
        <h2 className="text-lg font-bold text-navy-900">Erişim yetkiniz yok</h2>
        <p className="text-sm text-navy-500">Prim Takibi sayfası sadece admin hesaplarına açıktır.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-5">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3 pb-4 border-b border-navy-200">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-wider text-navy-400">Finans · Admin</p>
          <h1 className="mt-1 text-2xl font-bold text-navy-900">Prim Takibi — Tüm Personel</h1>
          <p className="mt-0.5 text-sm text-navy-500">
            {selectedMonth === "all" ? "Tüm aylar" : monthLabel(selectedMonth)} ·{" "}
            {selectedStaff === "all" ? "Tüm personel" : staffList.find((s) => s.id === selectedStaff)?.name || "—"} ·{" "}
            {totals.approved} onaylı dosya
          </p>
        </div>
        ...
        <div className="rounded-lg border border-navy-200 bg-white p-4">
          <div className="flex items-center justify-between">
            ...
            <svg className="w-4 h-4 text-navy-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9 12l2 2 4-4M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
      ...
      {/* Personel bazlı özet tablosu (yalnızca "Tüm Personel" görünümünde) */}
      {selectedStaff === "all" && perStaffTotals.length > 0 && (
        <div className="rounded-lg border border-navy-200 bg-white overflow-hidden">
          <div className="px-4 py-3 border-b border-navy-200 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-navy-900">Personel Bazlı Özet</h2>
            <span className="text-xs text-navy-500">{perStaffTotals.length} personel</span>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-[11px] font-semibold uppercase tracking-wider text-navy-500 bg-navy-50 border-b border-navy-200">
                  ...
                  <th className="px-4 py-2.5 text-right">Onaylı</th>
                  <th className="px-4 py-2.5 text-right">EUR</th>
                  <th className="px-4 py-2.5 text-right">USD</th>
                  <th className="px-4 py-2.5 text-right">TL</th>
                  <th className="px-4 py-2.5 text-right">Toplam (₺)</th>
                </tr>
              </thead>
              <tbody>
                {perStaffTotals.map((s) => (
                  <tr
                    key={s.id}
                    className="border-b border-navy-100 hover:bg-navy-50/50 transition-colors cursor-pointer"
                    onClick={() => setSelectedStaff(s.id)}
                  >
                    <td className="px-4 py-3 font-semibold text-navy-900">{s.name}</td>
                    <td className="px-4 py-3 text-right tabular-nums">{s.approved}</td>
                    <td className="px-4 py-3 text-right tabular-nums">
                      {s.eur > 0 ? `€${s.eur.toLocaleString("tr-TR", { maximumFractionDigits: 2 })}` : "—"}
                    </td>
                    <td className="px-4 py-3 text-right tabular-nums">
                      {s.usd > 0 ? `$${s.usd.toLocaleString("tr-TR", { maximumFractionDigits: 2 })}` : "—"}
                    </td>
                    <td className="px-4 py-3 text-right tabular-nums">
                      {s.tl > 0 ? `₺${s.tl.toLocaleString("tr-TR", { maximumFractionDigits: 0 })}` : "—"}
                    </td>
                    <td className="px-4 py-3 text-right font-bold text-navy-900 tabular-nums">
                      ₺{s.grandTl.toLocaleString("tr-TR", { maximumFractionDigits: 0 })}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="rounded-lg p-3 bg-amber-50 border border-amber-300 flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-amber-700 mt-0.5">
              Bu dosyalar için prim 0 olarak hesaplanır. Aşağıdakilere tıklayarak ekleyebilirsin.
            </p>
            <div className="flex flex-wrap gap-1.5 mt-2">
              {missingCountries.map((c) => (
                <button
                  key={c}
                  onClick={() => {
                    setRateDraft({ country: c, amount: "", currency: "EUR" });
      ...
      <div className="rounded-lg border border-navy-200 bg-white overflow-hidden">
        ...
        {monthEntries.length === 0 ? (
          <div className="p-12 text-center">
            <svg className="w-10 h-10 mx-auto text-navy-300 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
            <p className="text-sm text-navy-500">Bu seçimde prim kaydı yok.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
                ...
                {monthEntries.map((e) => {
                  ...
                  return (
                    <tr key={e.file.id} className="border-b border-navy-100 hover:bg-navy-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-navy-600 text-xs tabular-nums">
                        {formatDateTR(e.file.sonuc_tarihi)}
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`text-xs tabular-nums ${
                            isOverridden ? "text-primary-700 font-semibold" : "text-navy-700"
                          }`}
                        >
            ...
            <div className="grid grid-cols-12 gap-2">
              ...
              <div className="col-span-5 sm:col-span-2">
                <Select
                  ...
                  onChange={(e) => setRateDraft({ ...rateDraft, currency: e.target.value as "EUR" | "USD" | "TL" })}
                />
              </div>
              <div className="col-span-12 sm:col-span-2">
                <Button onClick={saveRate} className="w-full">
                  Kaydet
                </Button>
              </div>
            </div>
          ...
          <div className="rounded-lg border border-navy-200 bg-white max-h-[400px] overflow-y-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {rates.length === 0 && (
                  <tr>
                    <td colSpan={4} className="p-6 text-center text-navy-400 text-xs">
                      Henüz ülke oranı yok.
                    </td>
                  </tr>
                )}
                {rates.map((r) => (
                  <tr key={r.id} className="border-t border-navy-100 hover:bg-navy-50/50">
                    ...
                    <td className="px-3 py-2 text-right">
                      <div className="inline-flex gap-1">
                        <button
                          onClick={() =>
                            setRateDraft({ country: r.country, amount: String(r.amount), currency: r.currency })
                          }
          ...
          <div className="space-y-4">
            <div className="rounded-lg bg-navy-50 p-3 border border-navy-200">
              ...
              <p className="font-semibold text-navy-900">
                {editingFile.musteri_ad} — {editingFile.hedef_ulke}
              </p>
