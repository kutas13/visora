...
export default function CalendarPage() {
  ...
  const load = async () => {
    ...
    const [apptsRes, taleplerRes] = await Promise.all([apptsP, taleplerP]);
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              Randevu tarihlerini takvim üzerinde görüntüle · <span className="font-bold text-violet-600">{appointments.length}</span> yaklaşan
            </p>
          </div>
        </div>
        <div className="flex gap-1 bg-slate-100 rounded-lg p-0.5">
          {(["day","week","month"] as const).map(m => (
            <button key={m} onClick={() => setView(m)} className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${view===m ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
          ...
          {dayAppts.length === 0 ? (
            ...
            <div className="divide-y divide-slate-100">
              {dayAppts.map(a => {
                ...
                return (
                  <button key={a.id} onClick={() => onEdit(a)} className="w-full flex items-center gap-3 p-4 hover:bg-slate-50/50 transition-colors text-left">
                    ...
                    <div className="text-right flex-shrink-0">
                      <p className="text-xs text-slate-500">{a.profiles?.name}</p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-slate-50">
            {upcoming.slice(0,15).map(a => {
              ...
              const mine = a.assigned_user_id === uid;
              return (
                <button key={a.id} onClick={() => onEdit(a)} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-50/50 transition-colors text-left">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-slate-400">{a.hedef_ulke}{mine ? " · benim" : ""}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
