...
export default function MusteriDetayPage() {
  ...
  const stats = useMemo(() => {
    ...
    const totalUcret = files.reduce((acc, f) => {
      const u = Number(f.ucret || 0);
      return acc + u;
    }, 0);

    return { approved, rejected, pending, successRate, topCountry, totalUcret };
  }, [files]);

  const timeline = useMemo(() => {
    // Tüm dosyaların başlıca olaylarını zaman çizgisinde birleştir
    const events: { date: string; file: VisaFile; type: string; label: string; color: string }[] = [];
    for (const f of files) {
      events.push({
        date: f.created_at,
        file: f,
        type: "olustu",
        label: "Dosya oluşturuldu",
        color: "bg-blue-500",
      });
      if (f.basvuru_yapildi_at) events.push({
        date: f.basvuru_yapildi_at, file: f, type: "basvuru",
        label: "Başvuru yapıldı", color: "bg-purple-500",
      });
      if (f.sonuc && f.sonuc_tarihi) events.push({
        date: f.sonuc_tarihi, file: f, type: "sonuc",
  ...
  if (files.length === 0) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-3 text-center px-6">
        ...
        <button onClick={() => router.back()} className="text-sm text-primary-600 font-semibold hover:underline">Geri dön</button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      {/* Back */}
      <Link href="/app/musteriler" className="inline-flex items-center gap-1 text-xs font-semibold text-navy-500 hover:text-navy-900 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <h2 className="text-sm font-bold uppercase tracking-wider text-navy-500 px-1">Vize Dosyaları ({files.length})</h2>
          {files.map(f => {
            const statusLabel =
              f.sonuc === "vize_onay" ? "Onaylandı" :
              f.sonuc === "red" ? "Reddedildi" :
              f.islemden_cikti ? "İşlemden Çıktı" :
              f.basvuru_yapildi ? "Başvuru Yapıldı" :
              f.dosya_hazir ? "Dosya Hazır" : "Açık";
            const statusColor =
              f.sonuc === "vize_onay" ? "bg-emerald-100 text-emerald-700" :
              f.sonuc === "red" ? "bg-rose-100 text-rose-700" :
              f.basvuru_yapildi ? "bg-purple-100 text-purple-700" :
              f.dosya_hazir ? "bg-blue-100 text-blue-700" :
              "bg-navy-100 text-navy-700";
            return (
              <Link
                key={f.id}
                href={`/app/files/${f.id}`}
                className="block rounded-2xl border border-navy-200 bg-white p-4 hover:border-primary-300 hover:shadow-lg hover:shadow-primary-500/10 transition-all group"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-navy-500">
                      <span>Oluşturuldu: <b className="text-navy-700">{formatDate(f.created_at)}</b></span>
                      {f.randevu_tarihi && (
                        <span>Randevu: <b className="text-navy-700">{formatDate(f.randevu_tarihi)}</b></span>
                      )}
                      {f.sonuc_tarihi && (
                        <span>Sonuç: <b className="text-navy-700">{formatDate(f.sonuc_tarihi)}</b></span>
                      )}
                      {f.vize_bitis_tarihi && (
                        <span>Vize bitiş: <b className="text-navy-700">{formatDate(f.vize_bitis_tarihi)}</b></span>
                      )}
                    </div>
                  </div>
          ...
          <div className="rounded-2xl border border-navy-200 bg-white p-4">
            ...
            <div className="mt-3 space-y-2.5 text-sm">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold text-navy-900">{formatDate(files[files.length - 1].created_at)}</span>
              </div>
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold text-navy-900 tabular-nums">₺{stats.totalUcret.toLocaleString("tr-TR", { maximumFractionDigits: 0 })}</span>
              </div>
            </div>
          </div>
              ...
              {timeline.map((ev, idx) => (
                <div key={idx} className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-navy-500">{ev.file.hedef_ulke} · {formatDate(ev.date)}</p>
                  </div>
                </div>
              ))}
            </div>
