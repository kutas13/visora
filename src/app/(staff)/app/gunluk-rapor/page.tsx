    ...
    <div className="max-w-[1400px] mx-auto space-y-5">
      ...
      {activeTab === "gecmis" ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 space-y-3">
            ...
            {pastReports.length > 0 && (
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Müşteri adı veya tarih ile ara... (ör: Ali, 15.03)"
                  value={pastSearchTerm}
                  onChange={(e) => setPastSearchTerm(e.target.value)}
                  className="w-full pl-10 pr-8 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                />
                {pastSearchTerm && (
                  <button onClick={() => setPastSearchTerm("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
          ...
          {(() => {
            const q = normalize(pastSearchTerm.trim());
            const filtered = q
              ? pastReports
                  .map(r => {
                    const tarihMatch = r.tarih.replace(/-/g, ".").includes(pastSearchTerm.trim()) || r.tarih.includes(pastSearchTerm.trim());
                    const nameMatch = r.rows?.some(row => normalize(row.yolcuAdi || "").includes(q));
                    return { report: r, match: tarihMatch || nameMatch };
                  })
                  .filter(x => x.match)
                  .map(x => x.report)
              : pastReports;
            ...
            <div className="p-12 text-center">
              <p className="text-gray-500">{q ? "Aramayla eşleşen rapor bulunamadı" : "Henüz rapor gönderilmemiş"}</p>
            </div>
          ) : (
