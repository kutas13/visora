...
const PLATFORM_GROUPS: NavGroup[] = [
  {
    ...
    items: [
      ...
      {
        href: "/visora/references",
        label: "Referans Logoları",
        description: "Ana sayfada gösterilecek firma logoları",
...
export default function TopNav({ variant, userName = "Kullanıcı", orgName = "", avatarUrl = null }: TopNavProps) {
    ...
    >
      ...
      <div className="relative max-w-[1600px] mx-auto px-4 lg:px-8 h-16 flex items-center gap-4">
        ...
        <nav className="hidden lg:flex items-center gap-1 flex-1 ml-2">
          ...
          {/* Nasıl Kullanılır — bağımsız link */}
          {variant !== "platform" && (
            <Link
              href="/nasil-kullanilir"
              target="_blank"
              className={`px-3 py-2 text-[13px] font-medium rounded-lg transition ${
                pathname === "/nasil-kullanilir"
                  ? "bg-white/10 text-white"
                  : "text-slate-300 hover:text-white hover:bg-white/5"
              }`}
            >
          ...
          {/* Dilekçe AI — bağımsız link */}
          {variant !== "platform" && (
            <Link
              href="/dilekce-ai"
              className={`px-3 py-2 text-[13px] font-medium rounded-lg transition flex items-center gap-1.5 ${
                pathname === "/dilekce-ai"
                  ? "bg-white/10 text-white"
                  : "text-slate-300 hover:text-white hover:bg-white/5"
              }`}
        ...
        <div className="ml-auto flex items-center gap-1 lg:gap-2">
          ...
          <div className="relative">
            ...
            >
              <div className="relative w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-500 via-violet-500 to-fuchsia-500 flex items-center justify-center shadow-[0_6px_20px_-6px_rgba(99,102,241,0.6)] overflow-hidden">
                {avatarUrl ? (
              ...
              <div className="absolute right-0 top-full mt-2 w-60 rounded-2xl bg-white text-slate-900 shadow-[0_30px_80px_-20px_rgba(15,23,42,0.45)] ring-1 ring-slate-200/60 overflow-hidden z-50">
                ...
                <div className="p-1">
                  <Link
                    ...
                    className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] text-slate-700 hover:bg-slate-50 transition"
                  >
                    <Icon
                      d="M11 5h2m-1 0v14m-7 0h14M5 19V9a2 2 0 012-2h10a2 2 0 012 2v10M9 12h6m-6 4h6"
                      className="w-4 h-4"
                    />
                    Profili Düzenle
                  </Link>
