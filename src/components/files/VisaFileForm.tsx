...
import { TARGET_COUNTRIES, ISLEM_TIPLERI, EVRAK_DURUMLARI, PARA_BIRIMLERI, ODEME_PLANLARI } from "@/lib/constants";
import { createClient } from "@/lib/supabase/client";
import { notifyFileCreated, notifyFileUpdated } from "@/lib/notifications";
import type { VisaFile, IslemTipi, EvrakDurumu, ParaBirimi, OdemePlani } from "@/lib/supabase/types";
...
export default function VisaFileForm({ file, onSuccess, onCancel }: VisaFileFormProps) {
  ...
  const [odemePlani, setOdemePlani] = useState<OdemePlani>(file?.odeme_plani || "cari");
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-4">
            {ODEME_PLANLARI.map((plan) => (
              <label key={plan.value} className={`flex-1 p-4 border-2 rounded-xl cursor-pointer transition-all text-center ${odemePlani === plan.value ? (plan.value === "pesin" ? "border-green-500 bg-green-50" : "border-amber-500 bg-amber-50") : "border-navy-200 hover:border-navy-300"}`}>
                <input type="radio" name="odemePlani" value={plan.value} checked={odemePlani === plan.value} onChange={(e) => setOdemePlani(e.target.value as OdemePlani)} className="sr-only" />
                <span className={`font-medium ${odemePlani === plan.value ? (plan.value === "pesin" ? "text-green-700" : "text-amber-700") : "text-navy-700"}`}>
                  {plan.label}
                </span>
                <p className="text-xs mt-1 text-navy-500">
                  {plan.value === "pesin" ? "Ödeme alındı" : "Sonra tahsil edilecek"}
                </p>
              </label>
            ))}
          </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-4">
          {ISLEM_TIPLERI.map((tip) => (
            <label key={tip.value} className={`flex-1 p-4 border-2 rounded-xl cursor-pointer transition-all text-center ${islemTipi === tip.value ? "border-primary-500 bg-primary-50" : "border-navy-200 hover:border-navy-300"}`}>
